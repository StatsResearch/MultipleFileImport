...
We'll now create a large number of files and do some performance timing. I'll use the base R function Sys.time() to do the timing.
...
Status <- c('P','P','P','F','P','P','F','F','P','P')

test.df <- data_frame(Days,Units,Tens,Status)
...
```{r}
full.path.all.test.files <- paste0(Big.data.dir,'/',all.test.files)

start.time <- Sys.time()
all.file.results.3 <- lapply(full.path.all.test.files,GetSingleFileColumn)

all.file.results.3.df <- rbindlist(all.file.results.3)
end.time <- Sys.time()
elapsed <- (end.time - start.time)
elapsed

head(all.file.results.3.df)
```
...
## Clean up
We'll now clean up. Notice a couple of things here:

+ The use of paste0() with a single string and a character vector. This has the effect of 'tacking' on the single string
to each element of the character vector.

+ The use of status which confirms if the file removed worked.

```{r}
full.path.all.test.files <- paste0(Big.data.dir,'/',all.test.files)
status <- file.remove(full.path.all.test.files)
status
```
